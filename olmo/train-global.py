...
# [FILE]: olmo/train.py (Top Level Helper)

def get_lookahead_mask(seq_len: int, device: torch.device, debug_mode=False) -> torch.Tensor:
    """
    Generate Teacher Mask.
    debug_mode=False: Lookahead +1 (See next token).
    """
    if debug_mode:
        mask = torch.zeros((1, 1, seq_len, seq_len), device=device, dtype=torch.float)
        return mask

    # Standard Lookahead Mask
    indices = torch.arange(seq_len, device=device)
    # Mask condition: j > i + 1 (不能看 i+2 及以后)
    mask_bool = indices.view(1, -1) > (indices.view(-1, 1) + 1)
    
    mask = torch.zeros((seq_len, seq_len), device=device, dtype=torch.float)
    
    # [CRITICAL] 使用 -1e4 而不是 -inf 或 min_value，防止 fp16/bf16 下变成 NaN
    # Softmax(-10000) 已经是 0 了，足够安全
    mask.masked_fill_(mask_bool, -10000.0) 
    
    return mask.view(1, 1, seq_len, seq_len)

# [FILE]: olmo/train.py

def get_bidirectional_target_mask(seq_len: int, device: torch.device, debug_mode=False) -> torch.Tensor:
    """
    生成 Teacher 双向掩码：允许看过去和未来，但遮蔽当前预测目标。
    对于第 i 行（生成第 i+1 个 token 的状态），遮蔽第 i+1 列。
    """
    if debug_mode:
        return torch.zeros((1, 1, seq_len, seq_len), device=device, dtype=torch.float)

    indices = torch.arange(seq_len, device=device)
    
    # 核心逻辑修改：
    # Student 在位置 i 预测 i+1。因此 Teacher 在位置 i 时，必须遮蔽 column j = i + 1。
    # (i, j) 满足 j == i + 1 时触发 mask
    mask_bool = indices.view(-1, 1) == (indices.view(1, -1) - 1) 
    
    # 也可以写成：row i 对应 target index i+1
    # target_indices = indices + 1
    # mask_bool = (indices.view(-1, 1) == target_indices.view(1, -1))
    
    mask = torch.zeros((seq_len, seq_len), device=device, dtype=torch.float)
    # 使用 -10000.0 防止溢出 [cite: 312]
    mask.masked_fill_(mask_bool, -10000.0) 
    
    return mask.view(1, 1, seq_len, seq_len)

def compute_regret_loss_sum(student_logits, teacher_logits, labels, ignore_index=-100):
    """ 计算 KL 散度总和 """
    ...
    # 确保使用 float32 计算 KL，避免溢出
    student_log_probs = F.log_softmax(shift_student_logits.float(), dim=-1)
    teacher_probs = F.softmax(shift_teacher_logits.float(), dim=-1)

    kl_pointwise = F.kl_div(student_log_probs, teacher_probs, reduction='none')
    kl_per_token = kl_pointwise.sum(dim=-1)
    
    return (kl_per_token * valid_mask.float()).sum()

def create_packed_teacher_mask(
    ...
    """
    构造 Teacher Mask，同时满足：
    1. Lookahead: 允许 token i 看到 i+1 (j <= i + 1)
    2. Block Diagonal: 禁止跨文档注意力 (doc_id[i] == doc_id[j])
    
    Args:
        doc_lens: (Batch, MaxDocs) 每个 batch 中各个文档的长度
        seq_len: 序列总长度
    """
    bsz = doc_lens.size(0)
    
    # --- 1. 构造 Lookahead Mask (j > i + 1 时被 mask) ---
    indices = torch.arange(seq_len, device=device)
    # 允许 j <= i + 1, 即屏蔽 j > i + 1
    lookahead_mask_bool = indices.view(1, -1) > (indices.view(-1, 1) + 1) # Shape: (S, S)
    
    # --- 2. 构造 Document Block Mask ---
    # 我们需要为每个 token 生成一个 doc_id
    batch_doc_masks = []
    
    # 这是一个 CPU 操作循环，但 batch size 通常很小 (2, 4, 8)，开销可以忽略
    # 如果追求极致性能，可以写成纯 Tensor 操作，但逻辑会很复杂
    for i in range(bsz):
        # 获取当前序列中有效的文档长度
        lens = doc_lens[i]
        valid_lens = lens[lens > 0]
        
        # 生成 doc_ids: e.g. [3, 2] -> [0, 0, 0, 1, 1]
        # 使用 repeat_interleave 扩展
        if len(valid_lens) > 0:
            doc_ids = torch.repeat_interleave(
                torch.arange(len(valid_lens), device=device), 
                valid_lens
            )
        else:
            # 异常情况保护
            doc_ids = torch.zeros(seq_len, device=device, dtype=torch.long)
            
        # 处理 padding: 如果 sum(lens) < seq_len (通常 OLMo 是填满的，但以防万一)
        if len(doc_ids) < seq_len:
            pad_len = seq_len - len(doc_ids)
            # 给 padding 分配唯一的 ID，防止互相注意或注意正文
            pad_ids = torch.arange(len(valid_lens), len(valid_lens) + pad_len, device=device)
            doc_ids = torch.cat([doc_ids, pad_ids])
        
        # 截断（如果超长）
        doc_ids = doc_ids[:seq_len]
        
        # 构造该序列的 mask: doc_id 不相等的地方要 mask 掉
        # (S, 1) != (1, S) -> (S, S)
        doc_mask_bool = doc_ids.unsqueeze(1) != doc_ids.unsqueeze(0)
        batch_masks.append(doc_mask_bool)
        
    # Stack 成 (B, S, S)
    block_mask_bool = torch.stack(batch_masks)
    
    # --- 3. 合并 Mask ---
    # 最终 Mask 条件: (Lookahead 违规) OR (跨文档)
    # 广播 Lookahead: (1, S, S) | (B, S, S) -> (B, S, S)
    final_mask_bool = lookahead_mask_bool.unsqueeze(0) | block_mask_bool
    
    # 扩展为 (B, 1, S, S) 适配 Attention Head 维度
    final_mask = torch.zeros((bsz, 1, seq_len, seq_len), device=device, dtype=dtype)
    
    # 填充负无穷
    # 使用 min 值比 -1e4 更安全，确保 softmax 后绝对为 0
    final_mask.masked_fill_(final_mask_bool.unsqueeze(1), torch.finfo(dtype).min)
    
    return final_mask

# [FILE]: olmo/train.py

def create_packed_bidirectional_mask(
    doc_lens: torch.Tensor, 
    seq_len: int, 
    device: torch.device, 
    dtype: torch.dtype
    ...
    """
    构造 Packed Teacher Mask：
    1. Bidirectional: 允许在同文档内看未来。
    2. Target Masking: 遮蔽当前 row i 要预测的 target j=i+1。
    3. Document Isolation: 禁止跨文档注意力 [cite: 314, 321]。
    """
    bsz = doc_lens.size(0)
    indices = torch.arange(seq_len, device=device)
    ...
    # --- 1. 构造目标遮蔽 (j == i + 1) ---
    # 只有当 j 是 i 的直接后继时才遮蔽
    target_mask_bool = indices.view(-1, 1) == (indices.view(1, -1) - 1) # (S, S)
    
    # --- 2. 构造文档隔离掩码 (doc_id[i] != doc_id[j]) ---
    batch_masks = []
    for i in range(bsz):
        lens = doc_lens[i]
        valid_lens = lens[lens > 0]
        if len(valid_lens) > 0:
            doc_ids = torch.repeat_interleave(
                torch.arange(len(valid_lens), device=device), valid_lens
            )
        else:
            doc_ids = torch.zeros(seq_len, device=device, dtype=torch.long)
        ...
        doc_ids = doc_ids[:seq_len]
        # 不同文档 ID 之间互不可见 [cite: 321]
        doc_mask_bool = doc_ids.unsqueeze(1) != doc_ids.unsqueeze(0)
        batch_masks.append(doc_mask_bool)
        
    block_mask_bool = torch.stack(batch_masks) # (B, S, S)
    
    # --- 3. 合并掩码 ---
    # 最终遮蔽条件：(是预测目标) OR (跨文档) [cite: 322]
    final_mask_bool = target_mask_bool.unsqueeze(0) | block_mask_bool
    
    final_mask = torch.zeros((bsz, 1, seq_len, seq_len), device=device, dtype=dtype)
    final_mask.masked_fill_(final_mask_bool.unsqueeze(1), -10000.0)
...
try:
    ...
    def fused_loss_fn(
        ...
        # The `ignored_index` parameter of `cross_entropy_loss` was changed to `ignore_index` in v2.5.8
...
class Trainer:
    ...
    def __post_init__(self):
        ...
        # [NEW] Initialize regret loss tracker
        self._last_regret_loss = torch.tensor(0.0, device=self.device)
        self._last_teacher_ce = torch.tensor(0.0, device=self.device)
    ...
    def train_batch(
        ...
        regret_batch_loss = torch.tensor(0.0, device=self.device) # [NEW]
        teacher_ce_batch_loss = torch.tensor(0.0, device=self.device)
        ...
        current_regret_alpha = 0.0
        if hasattr(self.cfg, 'regret_alpha') and self.cfg.regret_alpha > 0.0:
            start_step = self.cfg.regret_start_step or 0
            # 如果还没到开始步数
            if self.global_step < start_step:
                current_regret_alpha = 0.0
            else:
                current_regret_alpha = self.cfg.regret_alpha

        do_regret = (current_regret_alpha > 0.0)
        ...
        for micro_batch_idx, micro_batch in enumerate(micro_batches):
            ...
            with grad_sync_context():
                with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                    ...
                    # Run standard forward (ce_loss is SUM here)
                    ce_loss_sum, z_loss_sum, student_logits = self.model_forward(
                        micro_batch, compute_z_loss=self.cfg.softmax_auxiliary_loss, loss_reduction="sum"
                    )
                    
                    # Normalize by GLOBAL batch size (standard OLMo practice)
                    ce_loss_norm = ce_loss_sum / batch_size_in_tokens
                    total_loss = ce_loss_norm

                    # Accumulate CE metrics
                    ce_batch_loss += ce_loss_norm.detach()
                    if z_loss_sum is not None:
                        z_loss_norm = z_loss_sum / batch_size_in_tokens
                        if self.cfg.softmax_auxiliary_loss:
                            total_loss += z_loss_norm
                        z_batch_loss += z_loss_norm.detach()

                    # --- 2. Handle MoE Loss (Student Only) ---
                    if self.model.config.block_type == BlockType.moe:
                        if self.model.config.moe_zloss_weight:
                            lb_loss, moe_z_loss = batched_load_balancing_loss(self.moe_args)
                            lb_loss = lb_loss / num_micro_batches
                            moe_z_loss = moe_z_loss / num_micro_batches
                            moe_z_batch_loss += moe_z_loss.detach()
                            total_loss += moe_z_loss
                        elif self.model.config.moe_loss_weight:
                            lb_loss = batched_load_balancing_loss(self.moe_args) / num_micro_batches
                        ...
                        # Clear stats immediately so Teacher pass doesn't pollute Router stats
                        clear_load_balancing_loss()

                    # --- 3. Teacher Forward (Regret Learning) ---
                    if do_regret:
                        ...
                        if doc_lens is not None:
                            # 使用新的双向 Packed 掩码函数
                            teacher_bias = create_packed_bidirectional_mask(
                                doc_lens, seq_len, self.device, target_dtype
                            )
                        else:
                            # Fallback 使用基础双向掩码
                            teacher_bias = get_bidirectional_target_mask(seq_len, self.device)


                        with torch.no_grad():
                            teacher_output = self.dist_model(
                                input_ids=micro_batch["input_ids"],
                                attention_mask=micro_batch.get("attention_mask"), 
                                attention_bias=teacher_bias, 
                                doc_lens=None, # [CRITICAL] Disable varlen internal masking
                                max_doc_lens=None,
                            )
                            teacher_logits = teacher_output.logits
                            # ==========================================
                            # [NEW] Calculate Teacher CE for Monitoring
                            # ==========================================
                            # 1. Get Labels (same as student)
                            t_labels = self.get_labels(micro_batch).view(-1)
                            # 2. Shift logits for prediction (remove last token)
                            t_logits = teacher_logits[..., :-1, :].contiguous().view(-1, teacher_logits.size(-1))
                            # 3. Compute CE Sum
                            t_ce_sum = F.cross_entropy(t_logits, t_labels, ignore_index=-100, reduction='sum')
                            # 4. Normalize by global batch tokens (same normalization as student)
                            t_ce_norm = t_ce_sum / batch_size_in_tokens
                            # 5. Accumulate
                            teacher_ce_batch_loss += t_ce_norm.detach()
                            # ==========================================
                            if self.model.config.block_type == BlockType.moe:
                                clear_load_balancing_loss()

                        # [DEBUG PRINT]
                        if self.global_step % 100 == 0 and micro_batch_idx == 0 and get_global_rank() == 0:
                             lbls_debug = self.get_labels(micro_batch).view(-1)
                             logs_debug = teacher_logits[..., :-1, :].contiguous().view(-1, teacher_logits.size(-1))
                             loss_fct_debug = torch.nn.CrossEntropyLoss(ignore_index=-100)
                             teacher_ce = loss_fct_debug(logs_debug, lbls_debug)
                             print(f"\n[DEBUG Step {self.global_step}] Student CE: {ce_loss_norm.item():.4f} | Teacher CE: {teacher_ce.item():.4f}")

                        labels = self.get_labels(micro_batch)
                        regret_loss_sum = compute_regret_loss_sum(student_logits, teacher_logits, labels)
                        regret_loss_norm = regret_loss_sum / batch_size_in_tokens
                        
                        # [MODIFIED] Apply dynamic alpha
                        total_loss += current_regret_alpha * regret_loss_norm
                        regret_batch_loss += regret_loss_norm.detach()
        ...
        # Store for logging in train_step
        self._last_regret_loss = regret_batch_loss
        self._last_teacher_ce = teacher_ce_batch_loss # <--- Store here
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        # [NEW] Retrieve Regret Loss
        regret_batch_loss = self._last_regret_loss
        teacher_ce_batch_loss = self._last_teacher_ce
        if reduce_global_loss:
            dist.reduce(ce_batch_loss, 0)
            ce_batch_loss.div_(get_world_size())
            
            # [NEW] Reduce Regret Loss
            if hasattr(self.cfg, 'regret_alpha') and self.cfg.regret_alpha > 0:
                dist.reduce(regret_batch_loss, 0)
                regret_batch_loss.div_(get_world_size())
        ...
        metrics["train/StudentCE"] = self.cur_train_loss # [Optional] Explicit alias for comparison
        # [NEW] Log Regret Metrics
        if hasattr(self.cfg, 'regret_alpha') and self.cfg.regret_alpha > 0:
            metrics["train/RegretLoss"] = regret_batch_loss.item()
            metrics["train/TeacherCE"] = teacher_ce_batch_loss.item() # <--- Log to Wandb
