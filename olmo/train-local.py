            ...
            with grad_sync_context():
                with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                    ...
                    if do_regret:
                        ...
                        # [MODIFIED] 使用新的 Packed Mask 构造函数
                        # 注意：我们必须从 micro_batch 中获取 doc_lens
                        # 如果数据中没有 doc_lens (非 packed 模式)，回退到简单的 lookahead mask
                        
                        doc_lens = micro_batch.get("doc_lens")
                        
                        if doc_lens is not None:
                            # 使用正确的 dtype (根据 Student 这里的 autocast context)
                            # 通常是 bfloat16 或 float16
                            target_dtype = torch.bfloat16 if self.cfg.autocast_precision == torch.bfloat16 else torch.float16
                            if self.cfg.autocast_precision == torch.float32:
                                target_dtype = torch.float32

                            teacher_bias = create_packed_teacher_mask(
                                doc_lens, seq_len, self.device, target_dtype
                            )
                        else:
                            # Fallback for non-packed data
                            teacher_bias = get_lookahead_mask(seq_len, self.device, debug_mode=False)
                            # 确保 dtype 正确
                            if teacher_bias.dtype != self.cfg.autocast_precision:
                                teacher_bias = teacher_bias.to(dtype=self.cfg.autocast_precision)
