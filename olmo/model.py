...
def lookahead_attention_bias(seq_len: int, device: torch.device) -> torch.FloatTensor:
    """
    [NEW] Creates a look-ahead bias mask allowing token i to attend to i+1.
    Resulting mask: 0 if j <= i + 1 else -inf
    """
    # diagonal=2 means we mask everything above the first super-diagonal (j >= i+2)
    att_bias = torch.triu(
...
def get_lookahead_attention_bias(cache: BufferCache, seq_len: int, device: torch.device) -> torch.Tensor:
    """
    [NEW] Cached version of lookahead bias.
    """
    if (bias := cache.get("lookahead_attention_bias")) is not None and bias.shape[-1] >= seq_len:
        if bias.device != device:
            bias = bias.to(device)
            cache["lookahead_attention_bias"] = bias
        return bias

    with torch.autocast(device.type, enabled=False):
        bias = lookahead_attention_bias(seq_len, device)
        cache["lookahead_attention_bias"] = bias
        return bias
...
class OLMoBlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        """
        Modified Attention: Enforce custom mask logic strictly.
        """
        # [CRITICAL] 强制关闭 causal 如果 mask 存在
        if attn_mask is not None:
            is_causal = False
        
        # [DEBUG PRINT ONCE] 确认 Mask 是否正确传入
        if attn_mask is not None and random.random() < 0.0001:
            # 打印 Mask 的一个小角: (0, 0) -> (0, 1) -> (0, 2)
            # Lookahead 应该是: 0, 0, -inf
            # Causal 应该是: 0, -inf, -inf
            try:
                # 假设 mask shape (1, 1, S, S) or (B, 1, S, S)
                m_slice = attn_mask[0, 0, 0, :5].detach().cpu().tolist()
                print(f"\n[ATTN INTERNAL] Mask slice [0,0,0,:5]: {m_slice}. is_causal={is_causal}")
            except:
                pass

        if max_doc_len is not None and cu_doc_lens is not None:
            assert attn_mask is None, "Custom mask not supported with varlen flash attn"
            # ... (Existing varlen logic) ...
            ...
            # ... (Existing standard flash attn logic) ...
            ...
            # [Regret Learning Path]
            # 手动处理 GQA
            if k.size(1) != q.size(1):
                k = k.repeat_interleave(q.size(1) // k.size(1), dim=1)
                v = v.repeat_interleave(q.size(1) // v.size(1), dim=1)
            
            # [CRITICAL] 确保 Mask 类型匹配 (bfloat16/float16 安全性)
            if attn_mask is not None:
                if attn_mask.dtype != q.dtype:
                    attn_mask = attn_mask.to(dtype=q.dtype)
                
                # 确保 Mask 形状可广播: (B, H, S, S)
                # Teacher mask 通常是 (1, 1, S, S)，PyTorch SDPA 可以自动广播，但显式检查更安全
                if attn_mask.dim() == 4 and attn_mask.shape[1] == 1:
                    pass # (1, 1, S, S) -> Broadcasting to (B, H, S, S) is supported
    ...
    def attention(
        ...
        if attention_bias is not None:
            # [FIX] 增加对 Look-ahead mask 的广播支持
            # 如果是 Teacher Mask (1, 1, S, S)，我们需要确保切片逻辑正确
            if attention_bias.shape[-1] >= key_len and attention_bias.shape[-2] >= query_len:
                 attention_bias = attention_bias[:, :, :query_len, :key_len]
            
            attention_bias = self._cast_attn_bias(attention_bias, dtype)

        # [FIX] 逻辑修正：只有当没有 explicit bias 时，才启用 is_causal
        # 对于 Teacher，attention_bias 不为空，所以 is_causal 为 False
        is_causal_logic = (attention_bias is None)
...
# [FILE]: olmo/model.py -> class OLMo
    ...
    def forward(
        ...
        if (
            ...
            # [FIX] 如果已经传入了 attention_bias (如 Teacher Mask)，则不要自动生成 Causal Mask
            ...
            # [FIX] 仅当 attention_bias 足够大时才进行切片，防止传入的 custom mask 被错误截断
            if attention_bias.shape[-1] >= mask_len:
                attention_bias = attention_bias[:, :, :mask_len, :mask_len].to(dtype=torch.float)

            # Add in the masking bias (Padding Mask).
            # [FIX] 即使是 Teacher Mask，也需要叠加 Padding Mask (忽略 Pad token)
            if attention_mask is not None:
        ...
        if self.config.block_group_size == 1:
            ...
            # Block groups logic (omitted for brevity, same change applies to 'attention_bias')
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                 if output_hidden_states:
                    all_hidden_states.append(x)
                 layers_past = None if past_key_values is None else past_key_values[...]
                 x, cache = block_group(
                    x,
                    attention_bias=attention_bias,
